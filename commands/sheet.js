    ...
    function authorize(credentials, callback) {
      ...
      function listMajors(auth) {
        ...
        sheets.spreadsheets.values.batchGet({
          spreadsheetId: '1D4qt2Iqpsj2ia-DCNQtNLNZLUlO9BpiMOaLd3b64NX8',
          ranges: ['July 2020!B2:K3', 'Roster!A:A'],
        }, (err, res) => {
          if (err) return console.log('The API returned an error: ' + err);
          const rows = res.data.valueRanges;
          if (rows.length) {
            
            const description = rows[0].values[0];
            const days = rows[0].values[1];
            const membersColumn = rows[1].values;
            const members = {name: []};
            
            for(member of membersColumn) {
